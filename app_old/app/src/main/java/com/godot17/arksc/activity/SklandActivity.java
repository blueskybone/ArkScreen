...
public class SklandActivity extends Activity {
    ...
    private void getGameInfo(JsonNode tree) {
         ...
         *  "stock": [
         *  "stockLimit": 10
         * ：(current - lastupdate) / ((completeTime - lastTime) / (limit - stock)) + stock
         * */
        ...
        int labor_current = 0;
        if (node_labor.get("remainSecs").asLong() == 0L) {
            labor_current = labor_update_value;
        } else {
            labor_current = (int) ((currentTs - node_labor.get("lastUpdateTime").asLong()) * (labor_max - labor_update_value)
                    / node_labor.get("remainSecs").asLong() + labor_update_value);
        }
