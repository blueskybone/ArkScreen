...
import com.blueskybone.arkscreen.data.network.NetWorkTask
import com.blueskybone.arkscreen.data.network.NetWorkTask.Companion.endfieldAttendance
        ...
        try {
            if (prefManager.autoAttendance.get()) {
                ...
                if (TimeUtils.getDayNum(currentTs) > TimeUtils.getDayNum(lastAttendanceTs)) {
                    ...
                    val accountEfDao = database.getAccountEfDao()
                    CoroutineScope(Dispatchers.IO).launch {
                        val accountList = accountSkDao.getAll()
                        val accountEfList = accountEfDao.getAll()
